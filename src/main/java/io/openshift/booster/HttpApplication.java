...
public class HttpApplication extends AbstractVerticle {
    ...
    public void start() {
        ...
        router.get(MESSAGE_SERVICE_API).handler(rc -> 
            rc.response()
                .putHeader(CONTENT_TYPE, "application/json; charset=utf-8")
                .end(new JsonObject()
                        .put("id","1")
                        .put("value","Greetings from Inside")
                        .encodePrettily()));
    ...
    private void helloHandler(RoutingContext rc) {
        ...
        RequestOptions requestOptions = 
            new RequestOptions()
                .addHeader("end-user", name)
                .setURI(MESSAGE_SERVICE_API);
        ...
        messageClient
            ...
            .as(BodyCodec.jsonObject()).rxSend()
                .map(resp -> {
                    String greetings = "No Greetings Available!";
                     
                    
                     if (resp.statusCode() != 200) {
                        LOGGER.error("Message Client Error when calling: status code {}",
                        resp.statusCode());
                        return new JsonObject().put("content", greetings);
                     }
                    
                     greetings = resp.body().getString("value");
                     return new JsonObject().put("content", String.format(messageTemplate,greetings, name, receiverName));
                }).subscribe(
    ...
    private void updateConfiguration() {
        ...
        messageClient = WebClient.create(vertx,
                new WebClientOptions().setDefaultHost(messageServiceHost)
                        .setDefaultPort(messageServicePort));
        LOGGER.info("New Message Service Endpoint retrieved: {}:{}", messageServiceHost,
                messageServicePort);
    }
}